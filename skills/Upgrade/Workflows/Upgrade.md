...
```bash
bun DEVOS/Tools/Upgrades.ts add --source upgrade-skill \
...
The store dedups by claim hash — re-running a scan never double-writes. Prior-Status grounding gains a source: check `bun DEVOS/Tools/Upgrades.ts list --json` for already-rejected (🚫) or already-applied (💬) claims alongside the existing `MEMORY/KNOWLEDGE/REJECTED/` check. Records surface in Pulse `/upgrades`; the applied half lands in the Ledger via `CreateUpdate.ts --upgrade-id`.
