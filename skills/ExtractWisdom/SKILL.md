...
`DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/ExtractWisdom/`
...
Reads a video, podcast, interview, or article first, works out which kinds of wisdom actually live in it, then invents sections to hold exactly those — never a fixed header set stamped onto everything. Five depth levels run from Instant to Comprehensive. Every full output closes with a one-sentence takeaway, an "If You Only Have 2 Minutes" list, and references worth chasing.

## Why Static Templates Flatten Content

Fixed extraction molds — IDEAS, QUOTES, HABITS, FACTS, every time — make a security talk and a business podcast produce the same skeleton, and the genuine gems get pressed into generic bullets. The result reads like a book report, not like a friend telling you the parts that stopped them cold. This skill lets the content dictate the sections and writes each point the way you'd say it out loud, so contrarian takes and first-time revelations come through undiluted instead of averaged away.

## The Adaptive Part

Sections are named for *this* content. A programming interview yields "Programming Philosophy" and "Developer Workflow Tips"; a business podcast yields "Contrarian Business Takes" and "Money Philosophy"; a security talk yields "Threat Model Insights" and "Defense Strategies." The content decides; the extraction follows.
...
**All levels share one voice and one quality bar.** Only the structure changes with depth — an Instant bullet should hit exactly as hard as a Comprehensive one.
...
**Canonical voice reference: `DEVOS/PROFILE/PRINCIPAL/WRITINGSTYLE.md`** — read this file for the full voice definition. The bullets should sound like the user telling a friend about it over coffee. Not compressed info nuggets. Not clever one-liners. Actual spoken observations.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"ExtractWisdom","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> DEVOS/MEMORY/SKILLS/execution.jsonl
```
