...
function resolveHarnessRoot(): string {
  ...
  if (existsSync(join(process.cwd(), "DEVOS", "SKILL.md"))) return process.cwd();
...
const PROJECTS_ROOT = join(HOME, ".claude", "projects"); // machine-global transcript store — harness-independent
...
function searchWorkJson(tokens: string[], since: Date | null, until: Date | null = null): Result[] {
  ...
  for (const [slug, entry] of Object.entries<any>(sessions)) {
    ...
    out.push({
      ...
      recencyDays: days,
      path: entry.isa ? (entry.isa.startsWith("/") ? entry.isa : join(DEVOS_DIR, entry.isa)) : undefined,
    });
  }
  out.sort((a, b) => {
...
async function searchProjectIsas(tokens: string[], since: Date | null, until: Date | null = null): Promise<Result[]> {
  ...
  for (const cwd of discoveredProjectCwds()) {
    if (cwd === DEVOS_DIR) continue; // task ISAs under ~/.claude are covered by the WORK_DIR sources
