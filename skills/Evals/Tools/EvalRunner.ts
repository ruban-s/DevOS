...
const DEVOS_ROOT = process.env.DEVOS_ROOT && existsSync(process.env.DEVOS_ROOT)
  ...
  : existsSync(join(process.cwd(), "DEVOS", "SKILL.md")) ? join(process.cwd(), "DEVOS") : join(homedir(), ".claude", "DEVOS");
const RESULTS_DIR = join(DEVOS_ROOT, "MEMORY", "STATE", "Evals-Results");
const USER_SUITES = join(DEVOS_ROOT, "PROFILE", "CUSTOMIZATIONS", "SKILLS", "Evals", "Suites");
...
export function buildLiveSystemPrompt(): string {
  const sources = [
    join(DEVOS_ROOT, "RUNTIME", "SYSTEM_PROMPT.md"),
    join(DEVOS_ROOT, "PROFILE", "OWNER.md"),
  ];
