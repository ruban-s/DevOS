...
`DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/Prompting/`
...
This skill builds prompts — it never delivers finished content. Whenever DevOS needs a prompt constructed, improved, or composed from data (a custom agent brief, an eval judge, a phased workflow), the work routes here. Structure lives in code, content lives in data, and what comes out is always a prompt destined for use somewhere else.

## Why Prompts Deserve a Library

Left to themselves, prompt patterns get hand-copied across every skill that needs them. Copies drift, best practices lodge in one person's head, and composing a prompt from data — minting a custom agent, assembling an eval judge, scaffolding a phased workflow — means gluing strings together with no separation between shape and substance. This library inverts that: one Handlebars template plus varying data renders agents, workflows, and eval frameworks on demand, with the engineering standards centralized where every skill can cite them.
...
- **Templates** — Handlebars machinery for programmatic prompt generation: Primitives (Briefing, Structure, Gate, Roster, Voice) plus eval templates (Judge, Rubric, TestCase, Comparison, Report). The agent-specific `DynamicAgent.hbs` lives in the Agents skill (`Agents/Templates/DynamicAgent.hbs`), not here.
...
import { renderTemplate } from '${DEVOS_SKILL_DIR}/Tools/RenderTemplate.ts';
...
The templating system eliminated **~35,000 tokens (65% reduction)** across DevOS:
...
**Philosophy:** Prompts that write prompts. Structure is code, content is data. Meta-prompting enables dynamic composition where the same template with different data generates specialized agents, workflows, and evaluation frameworks. This is core DevOS DNA - programmatic prompt generation at scale.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Prompting","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> DEVOS/MEMORY/SKILLS/execution.jsonl
```
