...
**When to Use**:
- Right after Deconstruct, to sort what's genuinely fixed from what merely claims to be
- When requirements feel tighter than the evidence justifies
- When "we can't do X" arrives with no supporting evidence
- For adversarial work (RedTeam, pentesting)
- Ahead of consequential architecture or strategy calls

---

## The Question Under Everything

Hold each stated constraint up to the same light:

> "Is this a law of physics, or a decision somebody made?"

Decisions can be unmade.

---

## Sort Every Constraint

Collect everything framed as a constraint — requirements, "we have to..." statements, best practices, budget and timeline walls, policy — then file each one:
...
A finished Challenge has three hallmarks:

- Each SOFT constraint traced back to its author, its original rationale, and whether that rationale still holds; each ASSUMPTION paired with its evidence and with a test capable of disproving it.
- The **removal test** run on every non-hard constraint: if striking it would unlock serious value, it belongs on the challenge list.
- The **unstated constraints** dragged into view — the ones so deeply assumed nobody voiced them ("obviously we need a database," "of course this needs auth"). Silence makes them the most dangerous kind.
