...
name: Optimize
version: 1.0.12
description: "Self-driving improvement loop — hill-climb anything. Code against metrics, or skills/prompts/agents against LLM-as-judge evals. USE WHEN optimize, hill climb, improve metric, reduce latency, optimize skill, optimize prompt, eval mode."
...
Point this skill at a target and it climbs: change the target, measure the outcome, keep wins, revert losses, repeat until the gains stop. Two instruments serve two kinds of target — metric mode for code that emits a number (latency, bundle size), eval mode for prose-like targets judged by LLM-as-judge binary evals (skills, prompts, agents).

## Why the Loop Beats Willpower

Tuning toward a measurable outcome by hand is slow, dull, repetitive labor. Edit a file, run the measurement, squint at whether it improved, keep or revert, repeat — dozens of rounds. Most people quit after a handful and freeze at "good enough," far below the real ceiling. Targets without a clean number (a skill's quality, a prompt's punch) are worse: no ready way exists to tell whether a change helped at all. This skill mechanizes the whole loop and retains only changes that win on the scoreboard.

## The Two Modes

One hill-climb loop, two measurement instruments:

- **Metric mode** — code targets measured by a shell command that emits a number (the original mode).
- **Eval mode** — skills, prompts, agents, or any text target scored by LLM-as-judge binary evals.

Lineage: Karpathy's [autoresearch](https://github.com/karpathy/autoresearch), extended with LLM-as-judge evaluation.
...
```
/optimize --target "DEVOS/skills/ExtractWisdom"
/optimize --target "DEVOS/skills/Research/Workflows/QuickResearch.md"
/optimize --target "prompts/my-prompt.md"
/optimize --target "DEVOS/skills/ExtractWisdom" --max-experiments 20
```
...
This skill steers the DevOS Algorithm as an autonomous mutation loop:
...
**Reference files:**
- `DEVOS/RUNTIME/ALGORITHM/optimize-loop.md` — the full loop protocol
- `DEVOS/RUNTIME/ALGORITHM/eval-guide.md` — how to write good eval criteria
- `DEVOS/RUNTIME/ALGORITHM/archive/target-types.md` — target detection and ISC generation
...
```
/optimize --target "DEVOS/skills/ExtractWisdom" --max-experiments 15
```
