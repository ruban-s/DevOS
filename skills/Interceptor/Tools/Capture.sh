...
USER_PREFS="${HOME}/.claude/DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/Interceptor/preferences.env"
...
#         DEVOS_DOWNLOADS_DIR overrides ~/Downloads when set — public PR #1535, @anikinsasha) ---
if [ -z "$OUT" ]; then
    ts="$(date +%Y%m%d-%H%M%S)"
    rand="$$"
    OUT="${DEVOS_DOWNLOADS_DIR:-${HOME}/Downloads}/interceptor-capture-${ts}-${rand}.png"
fi
mkdir -p "$(dirname "$OUT")"
...
guard_skipped() {
    ...
    local log="${HOME}/.claude/DEVOS/MEMORY/OBSERVABILITY/capture-guard.jsonl"
...
REMEDIATION (operator):
  ...
  Load Unpacked from DEVOS/skills/Interceptor/Extension/). If you just
...
REMEDIATION: reload the Interceptor extension (Load Unpacked from
  DEVOS/skills/Interceptor/Extension/), then retry.
EOF
        exit 10
    fi
else
