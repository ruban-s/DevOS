...
<!-- Tools/isa.ts parses THIS table. Column order is the contract:
     isc | type | check | threshold | tool | anchors_to | severity  (severity last, blank unless critical).
     `type` MUST be one of the real probe types: bash | bun-test | bun-property |
     test | curl | manual | screenshot | eval. An invented type or a non-table (YAML)
     block parses to ZERO probes — the exact zero-probe-green defect this example
     used to teach. Runnable types (bash/bun-test/bun-property/test/curl) count toward
