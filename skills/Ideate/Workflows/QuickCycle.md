...
**Reach for this when** full evolution is overkill and fast scored novelty will do. One cycle, no META-LEARN, no pivots, no Lamarckian feedback. Speed wins; depth is the price.

**Phase set:** CONSUME → STEAL → MATE → TEST. The perturbation stages (DREAM, DAYDREAM, CONTEMPLATE) and the iteration stages (EVOLVE, META-LEARN) are cut. Output: one batch of scored candidates.
...
- For each pair: crossover + dice-roll mutation (8 mutation operations from `../SKILL.md` § Beating Model Bias With Structure)
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"QuickCycle","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> DEVOS/MEMORY/SKILLS/execution.jsonl
```
