...
Phases execute **in order inside a cycle** — each consumes the previous phase's output. Agents **within** a phase run **concurrently** (Council pattern). Two mid-cycle checkpoints gate progress. The Loop Controller governs what happens at cycle boundaries.

### Phase 1: CONSUME (Ingest)

**Input:** Problem statement plus optional seeds. From cycle 2 onward, also EVOLVE's survivors, META-LEARN's research questions, and domain weight adjustments.
...
Each run persists to `DEVOS/MEMORY/WORK/{slug}/ideate/`. See `../SKILL.md` § "Where Run State Lives" for the full directory layout and idea data structure.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"FullCycle","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> DEVOS/MEMORY/SKILLS/execution.jsonl
```
