...
name: Ideate
version: 1.0.19
description: "Evolutionary idea engine — loop-driven multi-cycle generation that dreams, steals across domains, recombines, fitness-tests, selects, and meta-learns its way to ranked novel candidates with full provenance. USE WHEN ideate, id8, novel ideas, evolve ideas, dream up solutions, innovate, breakthrough ideas, idea evolution, multi-cycle creativity, need genuinely new approaches. NOT FOR quick single-pass brainstorming (use BeCreative)."
context: fork
background: false
---
...
`DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/Ideate/`

# Ideate — The Cognitive Progress Engine

## The Pitch

Ideate runs ideas through an evolutionary regime: large candidate pools, genetic material imported from unrelated fields, unsentimental selection, generation after generation. A Loop Controller paces the generations against a time budget; a Lamarckian Meta-Learner inspects which strategies actually paid off and steers the next cycle accordingly. What comes out the far end is a ranked slate of novel candidates, each one carrying its full lineage — parent ideas, the operation that produced it, the domains that fed it.

## Why a Single Pass Fails

Ask a model for ideas once and you receive the obvious set: riffs on one theme, pulled from the fat center of its training distribution. Turning up the temperature only reshuffles that same probability mass. Genuinely hard problems want material from somewhere else — a foreign discipline, an improbable recombination, a constraint flipped on its head — plus machinery that kills weak candidates, breeds the survivors, and repeats for many rounds. One pass structurally cannot deliver any of that.

## Not a Tool, a System

**Ideate is not BeCreative.** BeCreative widens a single pass. Ideate runs a multi-generation evolutionary process under loop control with Lamarckian meta-learning. Different jobs, different machinery.

## Five Functions Underneath

Human creativity reduces to five irreducible operations:
...
The nine phases below expand these into operable stages. DREAM, DAYDREAM, and CONTEMPLATE are PERTURB set to three different noise levels; MATE is PERTURB applied to surviving ideas; META-LEARN adds the Lamarckian edge — diagnosing *why* ideas succeeded and aiming the next generation accordingly.
...
After the loop stops, the Insight Extractor runs cross-cycle pattern analysis. Full phase mechanics live in `Workflows/FullCycle.md`.
...
After each cycle's META-LEARN phase, the controller rules continue / pivot / stop and owns all state between cycles:
...
## Structural Randomness, Not Temperature

LLM "temperature" is soft probability redistribution — still anchored to the training distribution. Ideate randomizes at the data level instead:

- **Input subsetting** (DREAM): a Fisher-Yates shuffle deals each agent its own input subset
- **Domain lottery** (STEAL): a weighted random draw from the 50+ candidate domain pool
- **Pairing shuffle** (MATE): Fisher-Yates pairs adjacent items; 20% of slots forcibly cross-phase
- **Mutation dice** (EVOLVE): roll an 8-sided die, apply the matching mutation:
...
Optional plugins that inject real-world signal into internal scoring:
...
Shipped hooks: `MarketSearch` (existing implementations), `FeasibilityCheck` (technical blockers), `ExpertPanel` (async human review), `PrototypeSimulation` (generate + test prototype).
...
Cycle count is the Loop Controller's adaptive call, never a fixed setting.

## Where Run State Lives

Each run persists under `DEVOS/MEMORY/WORK/{slug}/ideate/`:
...
When the Algorithm runs an ideation cycle it loads this skill and routes to `Workflows/FullCycle.md` by default. Tunable parameters from the algorithm's archived `DEVOS/RUNTIME/ALGORITHM/archive/parameter-schema.md` (historical — the mode system retired 2026-07-11) map to the configuration above. The Meta-Learner may adjust parameters within bounds; user-explicit overrides are auto-locked.
...
```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"Ideate","workflow":"WORKFLOW_USED","input":"8_WORD_SUMMARY","status":"ok|error","duration_s":SECONDS}' >> DEVOS/MEMORY/SKILLS/execution.jsonl
```
