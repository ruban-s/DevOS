...
Run a Fabric pattern inside the current session — no `fabric` subprocess. The pattern file is the prompt: open it, follow it, return what its output spec asks for.

---

## Workflow Steps

### Step 1: Match Intent to Pattern

Map what the user asked onto a pattern name:
...
**If the user names a pattern outright:** take them at their word (e.g., "use extract_article_wisdom" -> `extract_article_wisdom`). No second-guessing.

### Step 2: Open the Pattern File

Read the pattern's system.md file:

```bash
PATTERN_NAME="[selected_pattern]"
PATTERN_PATH="$HOME/.claude/DEVOS/skills/Fabric/Patterns/$PATTERN_NAME/system.md"
...
if [ -f "$PATTERN_PATH" ]; then
  ...
  ls DEVOS/skills/Fabric/Patterns/ | head -20
fi
```

### Step 3: Follow the Pattern Against the Content

**Native execution (the default path):**

A pattern's system.md is organized around:
- IDENTITY AND PURPOSE
- STEPS
- OUTPUT INSTRUCTIONS

Work through those instructions against the supplied content directly. The model reads and follows the pattern — nothing external runs.
...
### Step 4: Detours That Genuinely Need the Fabric CLI
...
**Pages that block direct reads (CAPTCHA, access walls):**
```bash
fabric -u "URL" -p [pattern_name]
```
Reach for this when native URL fetching fails.

### Step 4b: Auto-Harvest (summarize-family patterns)

**Trigger:** When the selected pattern belongs to the **summarize-family** AND the input is either a URL or text ≥200 characters, fire the knowledge-ingestion (harvest) skill on the SAME input as a side-effect of the summary — only if that skill is installed. Two outputs from one action: the summary in chat, the KNOWLEDGE note on disk.
...
Return whatever output format the pattern specifies. Most patterns define structured sections like:
...
**Pattern not found:**
```
Pattern '[name]' not found in DEVOS/skills/Fabric/Patterns/
