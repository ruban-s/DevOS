 ...
 * `MEMORY/` or nested under `DEVOS/MEMORY/`, so both are tried in order.
 */
...
function main(): void {
  ...
  const ratingsFile = resolveStore(arg("--ratings"), "SKILLSCAN_RATINGS_FILE",
    ["DEVOS/MEMORY/LEARNING/SIGNALS/ratings.jsonl", "MEMORY/LEARNING/SIGNALS/ratings.jsonl"], root, "ratings", warnings);
  const workDir = resolveStore(arg("--work"), "SKILLSCAN_WORK_DIR",
    ["DEVOS/MEMORY/WORK", "MEMORY/WORK"], root, "work", warnings);
