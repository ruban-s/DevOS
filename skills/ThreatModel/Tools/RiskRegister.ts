 ...
 *   default  DEVOS/PROFILE/SECURITY/THREATMODEL/
...
export function resolveDataDir(): string {
  let raw: string;
  if (process.env.THREATMODEL_DATA_DIR) raw = process.env.THREATMODEL_DATA_DIR;
  else if (process.env.DEVOS_ROOT) raw = join(process.env.DEVOS_ROOT, "PROFILE", "SECURITY", "THREATMODEL");
  else if (existsSync(join(process.cwd(), "DEVOS", "SKILL.md"))) raw = join(process.cwd(), "DEVOS", "PROFILE", "SECURITY", "THREATMODEL");
  else raw = join(homedir(), ".claude", "DEVOS", "PROFILE", "SECURITY", "THREATMODEL");
