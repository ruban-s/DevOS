...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: API_KEY not found in ${DEVOS_DIR}/.env');
    ...
    console.error('Error: Cannot read ${DEVOS_DIR}/.env');
    console.error('Create file: touch ${DEVOS_DIR}/.env');
    process.exit(1);
  }
}
```
...
CONFIGURATION:
  API Key: ${DEVOS_DIR}/.env (API_KEY=your_key)
...
For full documentation: DEVOS/Tools/${CLI_NAME}/README.md
Version: ${VERSION}
`);
}
```
...
- [ ] Configuration loading (from ${DEVOS_DIR}/.env)
