...
const PULSE_BASE = process.env.DEVOS_PULSE_BASE || "";
...
USAGE:
  bun DEVOS/skills/CMUX/Tools/cmux.ts <subcommand> [options]
...
function loadFleetConfig(): HostConfig[] | JsonObject {
  const configPath = join(homedir(), ".claude/DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/CMUX/fleet.json");
  if (!existsSync(configPath)) {
    return {
      ok: false,
      error: "No hosts configured. Pass --hosts name=ssh,name2=ssh2 or create DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/CMUX/fleet.json with {\"hosts\":[{\"name\":\"...\",\"ssh\":\"...\"}]}",
    };
  }
...
async function notifyVoice(message: string): Promise<boolean> {
  if (!PULSE_BASE) return false; // voice notifications need DEVOS_PULSE_BASE (Pulse is v2) — best-effort, never fatal
  try {
