...
```bash
DEVOS/MEMORY/WORK/{current_work}/
```

**To get the current work directory:**
1. Read `~/.claude/`
2. Extract the `work_dir` value
3. Use `DEVOS/MEMORY/WORK/{work_dir}/` for temporary artifacts
...
WORK_DIR=$(jq -r '.sessions | to_entries | map(select(.value.phase != "complete" and .value.mode != "native" and .value.mode != "starting")) | sort_by(.value.updatedAt) | reverse | .[0].key // empty' DEVOS/MEMORY/STATE/work.json)

# 2. Work in current work item directory
cd DEVOS/MEMORY/WORK/${WORK_DIR}/
...
3. **Use work item directory** - Work in `DEVOS/MEMORY/WORK/{work_dir}/`
