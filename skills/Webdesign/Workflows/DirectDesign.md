...
```
Read("DEVOS/skills/Webdesign/References/FrontendDesignPhilosophy.md")
```
...
If `DEVOS/PROFILE/CUSTOMIZATIONS/SKILLS/Webdesign/PREFERENCES.md` exists, read it after Step 0. Use it to bias (not bind) the register choice in Step 1. Without preferences, choose freshly each session and rotate registers across runs to avoid convergence on a single house style.
...
- **Saying `verified` without an Interceptor screenshot** — that is a doctrine violation in DevOS. The screenshot is the verification.
