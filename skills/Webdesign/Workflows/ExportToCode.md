...
```bash
OUT="${DEVOS_DOWNLOADS_DIR:-$HOME/Downloads}"/webdesign/export/$(date +%Y%m%d-%H%M%S)
mkdir -p "$OUT"

bun DEVOS/skills/Webdesign/Tools/DriveClaudeDesign.ts export bundle "$OUT/bundle"
```
...
```bash
bun DEVOS/skills/Webdesign/Tools/ProcessHandoffBundle.ts "$OUT/bundle" > "$OUT/bundle.json"
bun DEVOS/skills/Webdesign/Tools/ProcessHandoffBundle.ts "$OUT/bundle" --brief > "$OUT/integration-brief.md"
```
...
bun DEVOS/skills/Webdesign/Tools/VerifyDesign.ts http://localhost:5173 "$OUT/verify"
...
```bash
bun DEVOS/skills/Webdesign/Tools/VerifyDesign.ts --a11y http://localhost:5173 "$OUT/a11y"
```
