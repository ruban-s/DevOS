...
```bash
bun DEVOS/skills/Webdesign/Tools/DriveClaudeDesign.ts prompt "$(cat /tmp/brief.md)"
```
...
```bash
OUT="${DEVOS_DOWNLOADS_DIR:-$HOME/Downloads}"/webdesign/$(date +%Y%m%d-%H%M%S)
mkdir -p "$OUT"
bun DEVOS/skills/Webdesign/Tools/DriveClaudeDesign.ts screenshot "$OUT/v1.png"
```
...
```bash
bun DEVOS/skills/Webdesign/Tools/DriveClaudeDesign.ts export bundle "$OUT"
```

### 7. Verify

```bash
bun DEVOS/skills/Webdesign/Tools/VerifyDesign.ts "$OUT/index.html" "$OUT/verify"
```
