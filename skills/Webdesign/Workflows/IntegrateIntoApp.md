...
```bash
TARGET="$PROJECT_PATH"
OUT="${DEVOS_DOWNLOADS_DIR:-$HOME/Downloads}"/webdesign/integrate/$(date +%Y%m%d-%H%M%S)
mkdir -p "$OUT"
...
bun DEVOS/skills/Webdesign/Tools/VerifyDesign.ts "http://localhost:$DEV_PORT$INTEGRATION_TARGET" "$OUT/in-context"
