...
- Runtime is `bun >= 1.2` for users, who run `bun Tools/*.ts` directly and never `bun install`. Contributing needs `bun >= 1.4`: the committed `bun.lock` is lockfileVersion 2, which 1.2 cannot parse, so `bun install --frozen-lockfile` (and therefore the typecheck and CI) requires it. Tools are dry-run by default, `--apply` writes. Never node/npm/npx, and never a build step — the payload ships `.ts` run directly by bun, never compiled output.
